...
convertapi.convert('png', {'File': file_path}, from_format = 'pptx').save_files(folderPath)
...
min_moves_required = 2                                                          # Minimum number of moves required to perform a specific action; must be min_moves_required <= len_last_movements
...
while True:
    ...
    new_hCam = int(hCam * 0.6)                      # New image height as X% of the original height
    ...
    imgSlide = cv2.resize(imgSlide, (cap_width, cap_height))
    ...
    xMin, xMax = int(width * 0.4), int(width * 0.9)
    yMin, yMax = int(height * 0.2), int(height * 0.7)
    ...
    if hands and buttonPressed is False:
        ...
        if hand["center"][1] <= gestureThreshold and fingers == [1, 1, 1, 1, 1]:
            ...
            if sum(1 for movement in last_movements if movement == 'previous') >= min_moves_required:
                prev_hand_pos = None
            ...
            elif sum(1 for movement in last_movements if movement == 'next') >= min_moves_required:
                prev_hand_pos = None
                last_movements = ['nothing'] * len_last_movements
                buttonPressed = True
    ...
    # Display a small window in PowerPoint showing the camera view
    small_cam_percentage = 0.15                                                     # Percentage of the small camera dimensions relative to the PowerPoint image
    small_cam_width = int(imgSlide.shape[1] * small_cam_percentage)
    small_cam_height = int(imgSlide.shape[0] * small_cam_percentage)
    img_small_cam = cv2.resize(img, (small_cam_width, small_cam_height))
    h, w, _ = imgSlide.shape
    imgSlide[0:small_cam_height, w - small_cam_width: w] = img_small_cam
